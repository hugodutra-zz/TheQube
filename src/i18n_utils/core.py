...
def locale_name_to_LCID(locale_name):
 ...
 if func_locale_nameToLCID is not None:
		...
		LCID = func_locale_nameToLCID(unicode(locale_name), 0)
 else:
		LCList=[x[0] for x in locale.windows_locale.iteritems() if x[1]==locale_name]
		if len(LCList)>0:
			LCID=LCList[0]
		else:
			LCID=0
...
def available_languages(locale_dir, app_name):
 ...
 langs = sorted(['en'] + [i for i in dirs if os.path.isfile(os.path.join(locale_dir, '%s/LC_MESSAGES/%s.mo' % (i, app_name)))])
 langs = set(langs)
 result = dict()
 for l in langs:
