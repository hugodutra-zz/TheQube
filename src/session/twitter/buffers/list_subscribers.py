...
class ListSubscribers(Users):
 def __init__(self, owner=None, list=None, *args, **kwargs):
  ...
  self.item_name = _("list subscriber")
  self.item_name_plural = _("list subscribers")
  super(ListSubscribers, self).__init__(*args, **kwargs)
  self.init_done_event.set()

 def update(self, *args, **kwargs):
  self.clear()
  super(ListSubscribers, self).update(*args, **kwargs)

 def retrieve_update(self, *args, **kwargs):
  return self.cursored_update(update_function_name='get_list_subscribers', owner_screen_name=self.owner, slug=self.list['slug'])['users']
