...
class Migration (MigrationBase):
 ...
 def forwards(self):
  ...
  if 'buffers' in root['config']:
   for buffer_metadata in root['config']['buffers'].itervalues():
    if not hasattr(buffer_metadata, 'has_key'):
     continue
    if 'spoken' in buffer_metadata:
     buffer_metadata['spoken'] = self._Convert(buffer_metadata['spoken'])
    if 'clipboard' in buffer_metadata:
     buffer_metadata['clipboard'] = self._Convert(buffer_metadata['clipboard'])
